...
function Decision({ dec, setDec, tweaks }) {
          ...
          <thead>
            <tr>
              ...
              {dec.solutions.map(sol => (
                <th key={sol.id}>
                  <div className="col-head" data-picked={dec.pickedSolution === sol.id} onClick={() => pickSolution(sol.id)}>
                    <button
                      className="col-rm"
                      title="Remove solution"
                      onClick={(e) => { e.stopPropagation(); removeSolution(sol.id); }}
                    >×</button>
                    <div className="col-tag">{sol.tag} · {totals[sol.id]}/{maxPossible}</div>
                    ...
                    <span
                      className="pick"
                      title={dec.pickedSolution === sol.id ? "Picked" : "Pick this"}
            ...
            {dec.criteria.map(cr => (
              <tr key={cr.id}>
                ...
                {dec.solutions.map(sol => {
                  ...
                  return (
                    <td key={sol.id}>
                      <div className={"dec-cell" + (dec.pickedSolution === sol.id ? " picked-col" : "")}>
                        {tweaks.showRatings && (
                          <Stars
                            value={cell.rating}
...
function CardLayout({ dec, setCell, renameSolution, removeSolution, pickSolution, totals, maxPossible, tweaks }) {
  return (
    <div className="dec-table-wrap">
      <div className="dec-toolbar">
      ...
      <div className="dec-cards">
        {dec.solutions.map(sol => (
          <div key={sol.id} className="dec-card" data-picked={dec.pickedSolution === sol.id}>
            <div className="card-head">
              <div>
                <div className="card-tag">{sol.tag} · {totals[sol.id]}/{maxPossible}</div>
