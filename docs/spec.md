...
| FR-dec-17 | The Decision summary section header carries a **chosen-solution pill** that reflects `session.pickedSolution` live: when `revealed = true` it shows `✓ Chosen: <name>` (green) if a solution is picked, or `no solution chosen` (neutral) if none is. When `revealed = false`, it shows `— results hidden — press Reveal to view —` in place of the pill, so a pre-reveal pick does not leak the outcome. |
...
| FR-sync-15 | The renderer keeps its own bounded ledger of content hashes it has written. On `decisions://changed`, after re-fetching, it ignores the event when the fetched content hash is in that ledger **or** already equals the current `baseHash`. This is the correctness backstop to the best-effort Rust `self_writes` set (FR-sync-6): a duplicate or late echo of the app's own save never triggers a reload (FR-sync-9) or a conflict prompt (FR-sync-10) that could interrupt live typing. |
| FR-sync-16 | Autosave is concurrent-edit safe. The snapshot serialized to disk is reconciled against the live session by reference identity: if the user typed during the in-flight `save_session`, the newer edits are kept — only the freshly appended history is carried forward and `baseHash` advances — and the session stays dirty so the next debounce persists the newer edits. A stale snapshot never overwrites live edits (the prior bug where typing during a save was silently undone). |
